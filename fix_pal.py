...
# dict of tools that this script calls. The `None` values are replaced at
# runtime with each tool's absolute path if the tool is installed and executable.
tools = {
	'ffmpeg': None,
	'mkvextract': None,
	'mkvinfo': None,
...
def check_prereqs():
	...
	If any are missing, note which are and explain to the user that they must
	be installed."""
	global tools
	missing_tools = []
	for tool in tools:
...
def main():
	...
	with tempfile.TemporaryDirectory() as tmpdir:
		...
		# (No re-encoding necessary!) Adjust subtitle timings to match. Add the
		# adjusted chapters from the new chapter file. Write everything to a
		# temp file. (We'll see why in the following step.)
		...
		# This is why we used a temp file... we still have to re-encode audio
		# in order to keep the sample rate the same.
