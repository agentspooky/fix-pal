...
# Modify this if the correction factor needs tweaking. It's treated as a
# constant throughout.
CORRECTION_FACTOR = '25/24'


# dict of tools that this script calls. The `None` values are replaced
# at runtime with each tool's absolute path if the tool is installed and
# executable.
tools = {
    'ffmpeg': None,
    'mkvextract': None,
...
def check_prereqs():
    """For each utility needed, see if it's installed and grab its
    absolute path.

    If any are missing, note which are and explain to the user that they
    must be installed."""
    global tools
    missing_tools = []
    for tool in tools:
...
def check_exists(path):
    ...
    path = Path(path)  # lol - create a Path object out of the path string and store it in the same var
...
def confirm_overwrite(path):
    ...
    If the user does not confirm, or if the path points to a directory,
    quit."""
...
def adjust_timestamp(matchobj):
    ...
    new_timestamp = '{:02.0f}:{:02.0f}:{:02.9f}'.format(
        new_total_secs // 3600,
        new_total_secs % 3600 // 60,
        new_total_secs % 60)
    return new_timestamp


def edit_timecodes(infile, outfile):
    """Read an arbitrary track file, and produce a new one with
    timecodes adjusted.
...
def fix_chapters(infile, tmpdir):
    """Given an MKV file, pull out the existing chapters, then re-time
    them.
...
def get_sync_flags(infile):
    """Get info on NON-AUDIO tracks and build an array of `--sync` args
    for later use."""
    sync_args = []
    cmd = [tools['mkvmerge'], '-i', infile]
    stdout = subprocess.run(cmd, text=True, capture_output=True).stdout
...
def get_audio_sample_rate(file):
    ...
    If different tracks have different rates, choose that of the first
    audio track (technically, the first CHANNEL of the first audio
    track)."""
...
def fix_audio(infile, outfile):
    ...
    Copy video, subtitles, chapters exactly as they are in the input
    file."""
    ...
    cmd = [tools['ffmpeg'], '-y', '-i', infile, '-map', '0', '-filter:a',
           f'asetrate={sample_rate}*{audio_factor}', '-c:v', 'copy', '-c:s', 'copy',
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Get some info from the source file and build some of our args
        # for the mkvmerge call
        chapter_args = fix_chapters(infile, tmpdir)
        sync_args = get_sync_flags(infile)

        # For each video track, adjust the framerate by the correction
        # factor. (No re-encoding necessary!) Adjust subtitle timings to
        # match. Add the adjusted chapters from the new chapter file.
        # Write everything to a temp file. (We'll see why in the
        # following step.)
        ...
        # This is why we used a temp file... we still have to re-encode
        # audio in order to keep the sample rate the same.
